                    ...
                    <ul className='space-y-1 text-sm text-gray-700 max-h-40 overflow-y-auto'>
                        {yesVoters.map(voter => (
                            <li key={voter.id} className='truncate'>
                                <span className='font-medium'>{voter.name || 'N/A'}</span>: {voter.email}
                            </li>
                        ))}
                    </ul>
                </div>
...
// // --- NEW Component: Admin Voter List Display ---
// const AdminVoterList: React.FC<{ voterList: Voter[] }> = ({ voterList }) => {
    
//     const yesVoters = voterList.filter(v => v.voteType === 'YES');
//     const noVoters = voterList.filter(v => v.voteType === 'NO');

//     return (
//         <div className="mt-8 p-6 bg-indigo-50 border-t-4 border-indigo-300 rounded-xl shadow-inner">
//             <h5 className="text-lg font-bold text-indigo-800 mb-4 flex items-center gap-2">
//                 <Users className='w-5 h-5'/> Admin Voter Audit Log ({voterList.length} Total Votes)
//             </h5>
//             <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
//                 {/* YES Voters */}
//                 <div className='p-3 bg-white rounded-lg border border-green-200'>
...
//     )
// }
// // --- END NEW Component: Admin Voter List Display ---
...
//             if (type === 'CANCEL') {
//                 await onCancelVote();
//                 // Removed redundant message setting here, relying on success toast/parent update.
//             } else {
//                 await onVote(type); 
//                 // Removed redundant message setting here, relying on success toast/parent update.
//             }
...
//             {/* --- ADMIN VIEW ADDITION --- */}
//             {isAdmin && voterList.length > 0 && (
//                 <AdminVoterList voterList={voterList} />
//             )}
//             {/* --- END ADMIN VIEW ADDITION --- */}
//         </div>
//     );
