// hooks/use-drawing-deletion.ts
import { useState, useCallback } from 'react';
import { toast } from 'sonner';
import { useAction } from '@/hooks/use-action'; // Assuming this hook is correct
import { deleteDrawing } from '@/actions/delete-drawing'; // Assuming this action is correct
...
interface UseDrawingDeletionProps {
    currentUser: SafeUser | null | undefined;
    refreshDrawings: () => void; // Callback to refresh the list of drawings from the database
}

export const useDrawingDeletion = ({ currentUser, refreshDrawings }: UseDrawingDeletionProps) => {
    const [selectedFileToDelete, setSelectedFileToDelete] = useState<ImageUpload | null>(null);
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

    const { execute: executeDeleteDrawing, isLoading: isLoadingDelete } = useAction(deleteDrawing, {
        onSuccess: (data) => {
            toast.success(`File "${data?.cloudinaryPublicId}" deleted successfully.`);
            refreshDrawings();
        },
        onError: (error) => {
            toast.error(error);
    ...
    const handleDeleteConfirmation = useCallback(async () => {
        ...
        try {
            const publicId = selectedFileToDelete.cloudinaryPublicId;
            if (!publicId) {
            ...
            if (destroyResponse.ok) {
                executeDeleteDrawing({ id: selectedFileToDelete.id });
                setSelectedFileToDelete(null);
    ...
    }, [selectedFileToDelete, currentUser, executeDeleteDrawing, refreshDrawings]);
