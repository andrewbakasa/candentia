...
interface JobApplicationWithRelations extends JobApplication {
    ...
    jobAttachment?: JobAttachment[]; // Added to include attachments
}

interface JobApplicationContainerProps {
    jobApplication: JobApplicationWithRelations; // Changed type to JobApplicationWithRelations for type safety
    // Add onStatusChange prop: a function to call when the status is updated
    onStatusChange: (applicationId: string, newStatus: string) => void;
    currentUser?:SafeUser|null
}

export const JobApplicationContainer: React.FC<JobApplicationContainerProps> = ({ jobApplication, onStatusChange ,currentUser}) => {
    ...
    return (
        <Card className="hover:shadow-lg transition-shadow duration-200 h-full flex flex-col rounded-lg overflow-hidden">
            {/* The main content of the card, wrapped in a Link for navigation */}
                ...
                <div className=" p-4 pb-0 flex-grow"> {/* Added flex-grow to push dropdown to bottom if content is short */}
                    <CardHeader className="p-0 pb-2"> {/* Adjust padding to fit within the div */}
                        <CardTitle className="cursor-pointer text-xl font-semibold text-blue-700">  <Link href={`/job/${jobApplication.career?.id}`} passHref>
                            Application for: {jobApplication.career?.title || 'Unknown Job'}  </Link>
                        </CardTitle>
                    ...
                    <CardContent className="p-0"> {/* Adjust padding to fit within the div */}
                        <div className="space-y-2">
                            ...
                            {/* Display Job Attachments */}
                            {jobApplication.jobAttachment && jobApplication.jobAttachment.length > 0 && (
                                    ...
                                    <h4 className="text-md font-semibold text-gray-800 mb-2">Attachments:</h4>
                                         ...
                                         <Button
                                                    onClick={(e) => {
                                                        ...
                                                        e.preventDefault()
                                                        jobMediaModal.onOpen(jobApplication.id, jobApplication.career?.id||"", currentUser); // Use jobMediaModal and pass attachmentId and URL
                                                    }}
                                                    className="h-auto px-2 py-1.5 justify-end text-muted-foreground text-[11px] hover:text-sm"
                                                    size="sm"
                                                    variant="ghost"
                                                >
                                                    <AiFillEdit
