...
import { Enquiry, ProductEnquiry } from "@prisma/client";
import getCurrentUser from "./getCurrentUser";

export default async function getProductEnquiries() {
  try {
    const currentUser = await getCurrentUser();
    // No need to check if currentUser exists if you want to show public job openings

    let enquiry: ProductEnquiry[];

    if (currentUser?.isAdmin) {
      // Admin can view all active job openings
      enquiry = await prisma.productEnquiry.findMany({
        // where: {
        //   active: true,
        // },
