...
import prisma from "../../libs/prismadb"; // Assuming your Prisma client is exported from here
...
export async function POST(
  ...
  try {
    ...
    const {
      firstName,
      lastName,
      email, // This will be userEmail in the model
      membershipCategory,
      sex,
      country,
      profession,
    ...
    // --- IMPORTANT: Check for existing membership using userEmail ---
    const existingMembership = await prisma.membership.findUnique({
      where: {
        userEmail: email, // Use the incoming email to check against the unique userEmail field
      },
    });

    if (existingMembership) {
      // If a record with this email already exists, return a 409 Conflict status
      // and a clear message to the client.
      return new NextResponse(JSON.stringify({ message: 'A member with this email already exists. Please check your details or contact support.' }), { status: 409 });
    }
    ...
    const membership = await prisma.membership.create({
      data: {
        ...
        age: parsedAge,
        nextOfKin: nextOfKin,
        interests: interests, // Storing as String as per your model
    ...
    console.log("New membership created:", membership);
    return NextResponse.json(membership, { status: 201 }); // 201 Created for successful resource creation
