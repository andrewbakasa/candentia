...
interface InvestmentPortfolioFormProps {
    mockPortfolios: InvestmentPortfolio[];
}

const PortfolioInvestmentClient: React.FC<InvestmentPortfolioFormProps> = ({ mockPortfolios }) => {
    const [selectedPortfolio, setSelectedPortfolio] = useState<InvestmentPortfolioFormValues | null>(null);
    ...
    const handlePortfolioSelect = useCallback((portfolio: InvestmentPortfolio) => {
        // Map the portfolio data to match InputInvestmentClient's expected initialData type
        const initialData: InvestmentPortfolioFormValues = {
            id: portfolio.id,
...
0.00
export default PortfolioInvestmentClient;
