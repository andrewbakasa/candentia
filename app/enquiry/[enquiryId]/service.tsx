...
export const deleteMail = async (mailId: string): Promise<void> => {
    try {
        const response = await fetch(`/api/mail-delete/${mailId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export const cloneBOQ = async (id: string): Promise<void> => {
    try {
        const response = await fetch(`/api/boq-clone/${id}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        });
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || 'Failed to clone BOQ');
        }
        toast.success('BOQ cloned successfully');
    } catch (err: any) {
        console.error('Error cloning BOQ:', err);
        toast.error(err.message || 'Failed to clone BOQ');
        throw err;
    }
};
