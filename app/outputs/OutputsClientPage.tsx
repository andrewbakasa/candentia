...
// --- DESKTOP TABLE ROW COMPONENT (UPDATED) ---
...
// --- MOBILE CARD VIEW COMPONENT (UPDATED) ---
...
// --- PAGINATION CONTROLS COMPONENT (Kept) ---
...
const OutputsListPage: React.FC<OuputListClientProps> = ({
    ...
    // --- Filtering and Pagination Logic (UPDATED) ---
    const filteredOutputs = useMemo(() => {
        ...
        return outputs.filter(output => {
            ...
            // 2. Status Category Filtering (NEW)
            if (statusFilter === 'ALL') {
    ...
    useEffect(() => {
        setItemOffset(0);
    }, [searchTerm, pageSize, statusFilter]); // Added statusFilter dependency

    // --- PAGINATION HANDLERS (Kept) ---
    ...
    return (
        <div className="container mx-auto p-2 sm:p-4 lg:p-6 font-[Inter]"> 
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white py-3 px-0 sm:py-2 sm:px-0 border-b border-gray-200 mb-4"> 
                ...
                <button 
                    className={cn("px-4 py-2 text-sm font-semibold rounded-lg shadow-md transition", "bg-indigo-600 text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2")}
                    onClick={() => toast.info('Create Output action triggered (Placeholder)')}
                >
                    + Create New Output
                </button>
            </div>
...
// // Removed: FilterStatus type definition
// // Removed: ALL_STATUSES constant
...
// // Removed: getStatusClasses helper
...
//                     <h3 className="text-base font-semibold text-indigo-700 pr-6 max-w-[80%] truncate">
//                         {itemOffset + index+1}. {output.title}
//                     </h3>
...
//                 {/* Page Size Selector (Visible on desktop) */}
//                 <select
//                     className={cn('h-9 px-2 border border-gray-300 rounded text-sm focus:ring-1 focus:ring-indigo-500 cursor-pointer hidden sm:block', "bg-white")}
//                     value={pageSize}
//                     key={'pagesize-selector'}
...
//     // --- Filtering and Pagination Logic (SIMPLIFIED) ---
//     const filteredOutputs = useMemo(() => {
//         const lowerSearchTerm = searchTerm.toLowerCase().trim();
//         return outputs.filter(output => {
//             // Only search against title
//             const matchesSearch = 
//                 output.title.toLowerCase().includes(lowerSearchTerm); 
...
//                 {/* Search Input - Now uses full width/space previously taken by filter */}
