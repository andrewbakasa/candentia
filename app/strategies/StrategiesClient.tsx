...
const StrategyClient: React.FC<StrategyClientProps> = ({
    ...
    // 💡 NEW: State to track the currently expanded card ID
    ...
    // 💡 NEW: Handler to toggle card expansion
    const handleToggleExpand = (id: string) => {
        setExpandedCardId(prevId => {
            // If the clicked card is already open, collapse it (set to null)
            if (prevId === id) {
                return null;
            }
            // Otherwise, expand the clicked card and collapse the previous one
            return id;
        });
    };
