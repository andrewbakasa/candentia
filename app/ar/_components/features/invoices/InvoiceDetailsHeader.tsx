...
// Default the permission props to false if not passed by the parent component
const InvoiceDetailHeader: React.FC<InvoiceDetailHeaderProps> = ({ 
    invoice, 
    onDelete, 
    canEdit = false,       // <-- Defaulted to false
    canDelete = false,     // <-- Defaulted to false
}) => {
    ...
    return (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 sm:p-5 bg-white rounded-lg shadow-lg">
            ...
            <div className="flex flex-row gap-2 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0">
                ...
                {/* Mark Paid Button (Assuming this is a privileged/editable action) */}
                {canEdit && invoice.status !== InvoiceStatus.PAID && invoice.status !== InvoiceStatus.VOID && (
                    <button
                ...
                {canDelete && (
                    <ConfirmAction 
                        ...
                        description={`This action will permanently delete invoice ${invoice.invoiceNumber}.`}
                        showHint={false} 
                    />
                )}
            </div>
