// app/enquiries/EnquiresClient.tsx (Further Reduced Space)
/* eslint-disable @next/next/no-img-element */
'use client';
import { useCallback, useState, useEffect, useMemo } from "react"; 
import { useRouter } from "next/navigation";
import { SafeUser } from "../types";
// import Heading from "../components/Heading"; // REMOVED FOR SPACE
...
  const renderPaginationButtons = (showPageSize = true, showSummary = false) => {
    const buttons = [];
...
    // Summary (Only shown if requested by the call - not for desktop header or mobile footer now)
    if (showSummary) {
      buttons.push(
...
      
      {/* ------------------------------------------------------------------
        STICKY HEADER/TOOLBAR AREA (MAX COMPACT)
        ------------------------------------------------------------------ */}
      <div className="sticky top-0 w-full bg-white pt-0 pb-0 border-b border-gray-200 shadow-sm z-10"> {/* Tightened pt/pb */}
        <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            
            {/* Search Bar and Pagination */}
            <div className={cn("flex gap-4", isMobile ? 'flex-col' : 'flex-row items-center justify-between')}>
                
                {/* Search bar */}
...
                </div>
                
                {/* Pagination Controls - DESKTOP ONLY (Tighter margin/padding) */}
                <div className="flex-shrink-0 hidden sm:block flex items-center space-x-2"> {/* Added space-x-2 */}
                    {/* Pagination Summary (Moved to Stats Bar on desktop for compactness) */}
                    {renderPaginationButtons(true, false)} {/* showPageSize=true, showSummary=false */}
                </div>
            </div>

            {/* --- NEW INBOX STATS BAR (Single Row of Compact Info) --- */}
            <div className="flex justify-between items-center text-sm pt-0"> {/* Tighter pt-2 */}
                
                {/* LEFT SIDE: Total/Unread Count */}
                <div className="flex items-center space-x-4">
                    <p className="text-gray-600 font-medium whitespace-nowrap">
                        Total: <span className="text-gray-900">{totalEnquiries}</span>
                    </p>
                    {unreadEnquiries > 0 && (
...
                        <p className="text-green-600 font-medium whitespace-nowrap hidden sm:block">
                            (All read! 🎉)
                        </p>
                    )}
                </div>

                {/* RIGHT SIDE: Pagination Summary (Desktop Only) */}
...
      
      {/* ------------------------------------------------------------------
        ENQUIRIES LIST (TIGHTER PADDING/MARGINS)
        ------------------------------------------------------------------
    */}
      <div className={cn("mt-0 pb-5", )}> {/* Removed mt-2 */}
        <div>
          {
            (
              <div
                className={cn(
                  "grid grid-cols-1 space-y-0" // Removed the extra border-t here
                )}
              >
                {/* Ensure EnquiryRow handles its own top/bottom border if needed */}
...
        <div className="flex justify-between items-center px-4"> {/* Used justify-between to space out elements */}
            {/* Mobile Summary */}
...
            <div>
                {renderPaginationButtons(false)} {/* Pass false to hide page size selector/summary */}
            </div>
        </div>
    </div>
    
...
// // app/enquiries/EnquiresClient.tsx (Updated)
// /* eslint-disable @next/next/no-img-element */
// 'use client';
...
//             
//             {/* Search Bar and Pagination */}
//             <div className={cn("flex gap-4 pb-0", isMobile ? 'flex-col' : 'flex-row items-center justify-between')}> {/* Added pb-2 here */}
//                 
//                 {/* Search bar */}
//                 <div className="flex-grow max-w-lg w-full">
//                     <Search
