                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {contractsToDisplay.map((contract) => (
                            // Changed to link on the cell contents rather than the whole row to avoid accessibility issues with multiple links/buttons in a row,
                            // but maintained hover state for the row for visual feedback.
                            <tr key={contract.id} className="hover:bg-indigo-50/20 transition duration-150 ease-in-out">
                                ...
                                <td className="px-6 py-3 whitespace-nowrap">
                                    <Link href={`/contracts/${contract.id}`} className="block">
                                        <div className="text-sm font-semibold text-indigo-700 hover:text-indigo-900 transition">
                                            {contract.title}
                                        </div>
                ...
                {contractsToDisplay.map((contract) => (
                    ...
                    >
                        ...
                        <div className="flex justify-between items-start mb-2">
                            <h3 className="text-base font-semibold text-indigo-700 line-clamp-1 pr-6">{contract.title}</h3>
...
//   return (
//     <div className="container mx-auto p-1 sm:p-1 lg:p-2">
//       
//       {/* Header and Action Button: Adjust for better mobile spacing */}
//       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white py-3 px-4 sm:py-2 sm:px-6 border-b border-gray-200">
//     {/* Left side: Title and Description */}
//     <div className="flex flex-col mb-3 sm:mb-0">
//         {/* Reduced size from sm:text-4xl to sm:text-3xl */}
//         <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">
//             Contract Register
//         </h1>
//         {/* Reduced top margin on description */}
//         <p className="text-gray-500 text-sm sm:text-sm">
//             Overview of all active and historical contracts.
//         </p>
//     </div>
...
//     <Link 
//         href="/contracts/new" 
//         // Reduced vertical padding (py-1.5) and ensures full width on mobile for accessibility
//         className="bg-indigo-600 text-white flex items-center gap-2 px-5 py-1.5 rounded-lg shadow-md hover:bg-indigo-700 transition transform hover:scale-[1.01] active:scale-95 font-semibold text-sm w-full sm:w-auto justify-center"
//     >
//         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
//         New Contract
...
//     {/* --- Search and Filter Bar: Optimized for Space --- */}
// <div className="flex flex-col md:flex-row gap-2"> {/* Increased gap slightly for clarity when stacked */}
    
//     {/* Search Input */}
//     <div className="relative flex-grow md:w-8/12">
//         {/* Adjusted vertical alignment for the icon */}
//         <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
//         <input
//             type="text"
//             placeholder="Search by Title or Counterparty..."
//             value={searchTerm}
//             onChange={(e) => setSearchTerm(e.target.value)}
//             // Reduced py-3 to py-2 for a tighter fit
...
//             value={statusFilter}
//             onChange={(e) => setStatusFilter(e.target.value as FilterStatus)}
//             // Reduced py-3 to py-2 for a tighter fit. Changed rounded-xl to rounded-lg
//             className="w-full appearance-none py-2 px-4 border border-gray-300 rounded-lg bg-white text-gray-700 focus:ring-indigo-500 focus:border-indigo-500 transition shadow-sm cursor-pointer"
//         >
//             <option value="ALL">Filter by Status (All)</option>
//             {ALL_STATUSES.filter(s => s !== 'ALL').map(status => (
//                 <option key={status} value={status}>
...
//             ))}
//         </select>
//         {/* Added a subtle dropdown indicator for the select box */}
//         <svg className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
//     </div>
// </div>
