...
// import { useRouter } from 'next/navigation';
// import { ContractModel, ContractStatus } from '@prisma/client';
// // import { ContractStatus, ContractModel } from '@/types/contract';

// // Define the shape of data we send to the API (omitting read-only fields)
// type NewContractData = Partial<Omit<ContractModel, 
//   | 'id' 
//   | 'createdAt' 
//   | 'updatedAt' 
//   | 'internalOwner' 
...
// export default function NewContractPage() {
//   const router = useRouter();
//   const [formData, setFormData] = useState<NewContractData>({
//     title: '',
//     contractType: '',
//     counterpartyName: '',
//     status: ContractStatus.DRAFT, // Default status for new contracts
//     autoRenew: false,
//     // Note: Date and number fields can start as null or specific defaults
//     annualRevenueUsd: null,
//     internalOwnerId: 'placeholder_user_id', // MUST be replaced with actual user context/session ID
//   });
...
//     // Basic validation check
//     if (!formData.title || !formData.counterpartyName || !formData.internalOwnerId) {
//         setError('Title, Counterparty Name, and Internal Owner are required.');
//         setIsLoading(false);
//         return;
//     }

//     // 1. Prepare Data for API
//     // Ensure dates are correctly formatted or null if empty
//     const payload = {
//         ...formData,
...
//       <h1 className="text-3xl font-bold mb-6">✍️ Create New Business Contract</h1>
...
//           <div>
//             <label htmlFor="title" className="block text-sm font-medium text-gray-700">Title <span className="text-red-500">*</span></label>
//             <input type="text" name="title" id="title" value={formData.title || ''} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" />
//           </div>

//           {/* Contract Type */}
//           <div>
//             <label htmlFor="contractType" className="block text-sm font-medium text-gray-700">Contract Type</label>
//             <input type="text" name="contractType" id="contractType" value={formData.contractType || ''} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" placeholder="e.g., Vendor Agreement, Lease" />
//           </div>

//           {/* Counterparty Name */}
//           <div>
//             <label htmlFor="counterpartyName" className="block text-sm font-medium text-gray-700">Counterparty Name <span className="text-red-500">*</span></label>
//             <input type="text" name="counterpartyName" id="counterpartyName" value={formData.counterpartyName || ''} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" />
//           </div>

//           {/* Status (Default DRAFT, but allow selection) */}
//           <div>
//             <label htmlFor="status" className="block text-sm font-medium text-gray-700">Initial Status</label>
//             <select name="status" id="status" value={formData.status || ContractStatus.DRAFT} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3">
//               {Object.values(ContractStatus).map(status => (
//                 <option key={status} value={status}>{status.replace('_', ' ')}</option>
//               ))}
//             </select>
...
//         {/* --- Dates --- */}
//         <h2 className="text-lg font-semibold border-b pb-2 mt-4">Term Details</h2>
//         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          
//           <div>
//             <label htmlFor="effectiveDate" className="block text-sm font-medium text-gray-700">Effective Date</label>
//             <input type="date" name="effectiveDate" id="effectiveDate" value={formData.effectiveDate?.toISOString().split('T')[0] || ''} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" />
//           </div>
          
//           <div>
//             <label htmlFor="expirationDate" className="block text-sm font-medium text-gray-700">Expiration Date</label>
//             <input type="date" name="expirationDate" id="expirationDate" value={formData.expirationDate?.toISOString().split('T')[0] || ''} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" />
//           </div>

//           <div className="flex items-center pt-5">
//             <input type="checkbox" name="autoRenew" id="autoRenew" checked={formData.autoRenew} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded" />
//             <label htmlFor="autoRenew" className="ml-2 block text-sm font-medium text-gray-700">Auto Renew?</label>
//           </div>

//         </div>

//         {/* --- Financials --- */}
//         <h2 className="text-lg font-semibold border-b pb-2 mt-4">Financial Projections (Section 2: Financials)</h2>
//         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          
//           <div>
//             <label htmlFor="annualRevenueUsd" className="block text-sm font-medium text-gray-700">Annual Revenue (USD)</label>
//             <input type="number" name="annualRevenueUsd" id="annualRevenueUsd" value={formData.annualRevenueUsd || ''} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" placeholder="0.00" />
//           </div>
          
//           <div>
//             <label htmlFor="annualizedCostUsd" className="block text-sm font-medium text-gray-700">Annualized Cost (USD)</label>
//             <input type="number" name="annualizedCostUsd" id="annualizedCostUsd" value={formData.annualizedCostUsd || ''} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" placeholder="0.00" />
//           </div>
          
//           <div>
//             <label htmlFor="riskRating" className="block text-sm font-medium text-gray-700">Risk Rating (1-5)</label>
//             <input type="number" name="riskRating" id="riskRating" value={formData.riskRating || ''} onChange={handleChange} min="1" max="5" step="0.1" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3" placeholder="e.g., 3.5" />
//           </div>
//         </div>
...
//           <textarea name="description" id="description" rows={3} value={formData.description || ''} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3"></textarea>
//         </div>
...
//             type="submit"
//             disabled={isLoading}
//             className="bg-green-600 text-white px-6 py-3 rounded-md hover:bg-green-700 disabled:bg-gray-400 font-semibold transition"
//           >
