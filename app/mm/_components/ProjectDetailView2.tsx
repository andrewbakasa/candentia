                            ...
                            <>
                                <table className="w-full text-left">
                                    ...
                                    {project.materialRequirements?.map((mat: any) => {
                                        // 1. Log here
                                        console.log('mat detail:', mat);

                                        // 2. Explicitly return the JSX
                                        return (
                                        <tr key={mat.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-8 py-4 font-bold text-slate-700">{mat.material.description} {mat.material.itemCode}</td>
                                            ...
                                            <button 
                                                onClick={() => { setEditingRecord(mat); setIsBoQModalOpen(true); }} 
                                                className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-indigo-600"
                                            >
                                                <Edit2 size={14}/>
                                            </button>
                                            <button 
                                                onClick={() => handleDelete(mat.id, 'materials')} 
                                ...
                                {/* Requested row/div below BoQ registry */}
                                <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                                    <p className="text-[10px] font-bold text-slate-500 uppercase">Registry Summary</p>
                                    <p className="text-xs font-black text-slate-700">Estimated Total: ${totalBoQValue.toLocaleString()}</p>
                                </div>
                            </>
                        ) : (
                            <table className="w-full text-left">
                                        ...
                                        console.log('PO',po)
                                         return (
                                          <tr key={po.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-8 py-4 font-black text-slate-900">{po.poNumber}</td>
                                            ...
                                            <td className="px-8 py-4 text-right space-x-2">
                                                <button onClick={() => { setEditingRecord(po); setIsPOModalOpen(true); }} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-indigo-600"><Edit2 size={14}/></button>
                ...
                <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="w-full max-w-2xl bg-white rounded-t-[2rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[92vh] flex flex-col animate-in slide-in-from-bottom duration-300">
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            ...
                            <button onClick={() => { setIsTaskModalOpen(false); setIsModalOpen(false); setIsPOModalOpen(false); setIsBoQModalOpen(false); }} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
