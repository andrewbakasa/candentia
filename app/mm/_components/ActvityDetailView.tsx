...
import { 
    ...
    Trash2, RefreshCcw, Filter, Eye, EyeOff, Sparkles
} from 'lucide-react';
...
interface ActivityDetailViewProps {
    activity: any;
    baseTasks: any[]; // The master list of standardized base tasks
    onRefresh?: () => void;
    currentUser: SafeUser | null;
    MM_TaskForm_Component?: React.ComponentType<{
        initialData?: any;
        activities: any[];
        baseTasks: any[]; // Prop added here
...
export default function ActivityDetailView({ 
    ...
    const filteredTasks = useMemo(() => {
        ...
        if (!showPendingOnly) return activity.tasks;
        return activity.tasks.filter((task: any) => task.status !== 'COMPLETED');
    }, [activity.tasks, showPendingOnly]);
    ...
    const handleDispatchOrder = () => {
        // Logic: Check if the current activity matches a standardized base task name
        const match = baseTasks.find(bt => 
            bt.standardTitle?.toLowerCase() === activity.description?.toLowerCase()
        );

        setEditingTask({
            title: match?.standardTitle || `WO: ${activity.description}`,
            assignedTo: activity.supervisor || '',
            status: 'OPEN',
            description: match?.standardDesc || `Execution for Activity ID: ${activity.id.slice(-6)}`,
            // If the base task has standardized costs/hours, they can be pre-filled here
            estimatedHours: match?.benchmarkHours || 0
        });
        setIsTaskModalOpen(true);
    };
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                        <div className="space-y-4">
                            <div className="flex items-center gap-2">
                                ...
                                {baseTasks.some(bt => bt.standardTitle === activity.description) && (
                                    <span className="bg-emerald-100 text-emerald-600 text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest flex items-center gap-1">
                                        <Sparkles size={10} /> Base Protocol Verified
                                    </span>
                                )}
                            </div>
                ...
                <div className="p-3 md:p-4 flex flex-col md:flex-row items-center justify-between border-b border-slate-50 bg-slate-50/20 gap-4">
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-bold tracking-wide">
                                {showPendingOnly ? 'Viewing: Pending Actions' : 'Viewing: All Dispatches'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <button 
                            ...
                            className={`flex items-center gap-2 px-5 py-3 rounded-2xl transition-all text-[10px] font-black uppercase tracking-widest border ${
                                showPendingOnly 
                                ? 'bg-amber-500 border-amber-600 text-white shadow-lg' 
                                : 'bg-white border-slate-200 text-slate-500 hover:border-indigo-300'
                            }`}
                        >
                            {showPendingOnly ? <EyeOff size={16} /> : <Eye size={16} />}
                            {showPendingOnly ? "Pending Only" : "Show All"}
                        </button>

                        <button 
                            onClick={handleDispatchOrder}
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-slate-900 hover:bg-indigo-600 text-white rounded-2xl shadow-xl transition-all text-xs font-black uppercase tracking-widest"
                        >
                            <Plus size={18} />
                            <span>Dispatch</span>
                        </button>
                    ...
                    {filteredTasks.length > 0 ? (
                        ...
                        <div className="py-20 text-center text-slate-300 border-2 border-dashed border-slate-100 rounded-[2rem]">
                            ...
                            <p className="text-xs font-bold uppercase tracking-widest">
                                {showPendingOnly ? "No Pending Work Orders Found" : "No Work Orders Dispatched"}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
                    <div className="w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
                        <TaskForm 
                            ...
                            activities={[activity]} 
                            baseTasks={baseTasks} // Passing to standardized form
...
function TaskCard({ task, onEdit, onDelete, isDeleting, isAllowedDelete, isAllowedEdit }: any) {
    ...
    return (
        <div className={`
            relative p-4 rounded-[2rem] border transition-all group
            ${isDeleting ? 'opacity-50 pointer-events-none' : ''}
            ${isCompleted 
                ? 'bg-emerald-50/30 border-emerald-100 shadow-sm' 
                : 'bg-white border-slate-200 shadow-md hover:shadow-xl hover:border-indigo-200'}
        `}>
            <div className="flex justify-between items-start mb-6">
                <span className={`flex items-center gap-1 text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest ${
                    ...
                    {isCompleted ? <ShieldCheck size={10} /> : <Clock size={10} />}
                    {task.status}
                </span>
                ...
                <div className="flex items-center gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {isAllowedDelete && (
                        <ConfirmAction 
                            onConfirm={onDelete} itemId={task.id} action="Delete" heading="Confirm Deletion"
            ...
            <h3 className={`text-sm font-black mb-2 truncate transition-colors ${
                isCompleted ? 'text-slate-500 line-through' : 'text-slate-800 group-hover:text-indigo-600'
            }`}>
                {task.title}
            </h3>
            
            <p className={`text-[10px] font-medium line-clamp-2 mb-6 italic ${
            ...
            <div className={`flex items-center justify-between pt-4 border-t ${
                isCompleted ? 'border-emerald-100' : 'border-slate-50'
            }`}>
                <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-full border-2 border-white flex items-center justify-center shadow-sm text-[8px] font-black uppercase ${
                        isCompleted ? 'bg-emerald-200 text-emerald-700' : 'bg-slate-100 text-slate-500'
                    }`}>
...
function StatCard({ icon, label, value, color }: any) {
    ...
    return (
        <div className={`p-5 rounded-[2rem] border ${theme[color]} shadow-sm`}>
            <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest mb-1 opacity-60">{icon} {label}</div>
            <div className="text-xl font-black tracking-tight">{value}</div>
        </div>
    );
}

// ... StatCard and TaskCard components remain as provided in previous block
// 
...
//     Target, TrendingUp, AlertTriangle, ShieldCheck, 
//     Activity as ActivityIcon, User, ArrowLeft, LayoutList,
//     Trash2, RefreshCcw, Filter, Eye, EyeOff
// } from 'lucide-react';
// import toast from 'react-hot-toast';
// import axios from 'axios';
// import MM_TaskForm from './TaskForm';
...
//         if (!activity.tasks) return [];
//         if (!showPendingOnly) return activity.tasks;
//         // Filter out COMPLETED tasks to show only pending/in-progress ones
//         return activity.tasks.filter((task: any) => task.status !== 'COMPLETED');
//     }, [activity.tasks, showPendingOnly]);
...
//             {/* --- STRATEGIC OVERVIEW GRID (Header & Budget) --- */}
//             <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
//                 <div className="lg:col-span-8 bg-white rounded-[2.5rem] p-6 md:p-10 border border-slate-200 shadow-sm relative overflow-hidden">
//                     <div className="flex flex-col md:flex-row justify-between items-start gap-6">
//                         <div className="space-y-4">
...
// // --- SUB-COMPONENTS (Keep existing TaskCard and StatCard implementation) ---
// function TaskCard2({ task, onEdit, onDelete, isDeleting, isAllowedDelete, isAllowedEdit }: any) {
//     return (
...
//                 </span>
//                 <div className="flex items-center gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
//                     {isAllowedEdit && <button onClick={onEdit} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"><Edit3 size={14}/></button>}
//                     {isAllowedDelete && <ConfirmAction 
//                         onConfirm={onDelete} itemId={task.id} action="Delete" heading="Confirm Deletion"
//                         description="This action will permanently remove this item."
//                         triggerButton={<button className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors"><Trash2 size={16} /></button>}
//                     />}
//                 </div>
//             </div>
//             <h3 className="text-sm font-black text-slate-800 mb-2 truncate group-hover:text-indigo-600 transition-colors">{task.title}</h3>
//             <p className="text-[10px] text-slate-400 font-medium line-clamp-2 mb-6 italic">{task.description || 'Standard maintenance protocol applies.'}</p>
//             <div className="flex items-center justify-between pt-4 border-t border-slate-50">
//                 <div className="flex items-center gap-2">
//                     <div className="w-8 h-8 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center text-slate-500 shadow-sm text-[8px] font-black uppercase">{task.assignedTo?.substring(0, 2) || 'OP'}</div>
//                     <span className="text-[9px] font-black text-slate-600 uppercase">{task.assignedTo || 'Open Pool'}</span>
//                 </div>
//                 <span className="text-[9px] font-bold text-slate-300">ID-{task.id.slice(-4)}</span>
//             </div>
//         </div>
...
//                 : 'bg-white border-slate-200 shadow-md hover:shadow-xl hover:border-amber-200'}
//         `}>
//             {/* Status Badge */}
//             <div className="flex justify-between items-start mb-6">
//                 <span className={`flex items-center gap-1 text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest ${
...
//             {/* Subtle "Done" Checkmark Watermark for completed tasks */}
//             {isCompleted && (
//                 <ShieldCheck className="absolute right-4 bottom-4 text-emerald-500/10 w-12 h-12 -rotate-12" />
//             )}
//         </div>
//     );
// }
