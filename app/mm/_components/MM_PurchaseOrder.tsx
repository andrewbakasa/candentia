...
import { X, ShieldCheck, Box, AlertCircle, Receipt, Loader2, Trash2, Store, Info, Hash } from 'lucide-react';
        ...
        body: JSON.stringify({
          ...
          lineItems: selectedItems.map((item: any) => {
            ...
            return {
                ...
                itemCode: materialData.itemCode || "N/A", 
                description: materialData.description || item.description || "No Description",
      ...
      <form onSubmit={handleSubmit} className="flex-1 flex overflow-hidden">
        <div className="w-80 border-r p-6 space-y-6 bg-white overflow-y-auto">
          <div className="space-y-5">
            ...
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">2. Strategy Authorization</label>
            ...
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">3. Vendor / Supplier</label>
              <input className="w-full p-3 bg-slate-100 rounded-xl text-xs font-bold border-none" value={vendorName} onChange={(e) => setVendorName(e.target.value)} required />
            </div>

            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">4. Maintenance Project</label>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
            {!selectedProjectId ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-400">
                <p className="text-[10px] font-black uppercase">No Material Requirements Found</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className={`text-xs font-black ${isSelected ? 'text-indigo-900' : 'text-slate-700'}`}>
                          {m.material?.description || 'No Description'}
                        </p>
                        <p className="text-[10px] font-mono text-slate-400 uppercase tracking-tighter">
                          {m.material?.itemCode} • Qty: {m.quantityRequired} {m.material?.unitOfMeasure || 'units'}
                        </p>
                      </div>
                    </div>
          ...
          <div className="mt-6 pt-6 border-t flex justify-between items-center bg-slate-50/50">
             <div className="flex items-center gap-6">
                <div>
                  ...
                  <p className={`text-2xl font-black tracking-tighter ${totalCommitment > (selectedStrategy?.totalBudget || Infinity) ? 'text-red-600' : 'text-slate-900'}`}>
                      ${totalCommitment.toLocaleString()}
                  </p>
                </div>
             </div>
             <button type="submit" disabled={loading || selectedReqIds.length === 0} className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2">
...
//   // IMPORTANT: availableMaterials now contains requirements with nested .material
//   const availableMaterials = activeProject?.materialRequirements || [];
...
//         //   lineItems: selectedItems.map((item: any) => ({
//         //     requirementId: item.id,
//         //     // UPDATED: Mapping from normalized material relation
...
//         //   }))
//            lineItems: selectedItems.map((item: any) => {
//             // Defensive check: Ensure the nested material object exists
//             const materialData = item.material || {};
            
//             return {
//                 requirementId: item.id,
//                 // Provide fallbacks to avoid 'undefined'
//                 itemCode: materialData.itemCode || "N/A", 
//                 description: materialData.description || item.description || "No Description",
//                 quantity: item.quantityRequired,
//                 unitPrice: item.estimatedUnitCost
...
//         {/* Sidebar unchanged */}
//         <div className="w-80 border-r p-6 space-y-6 bg-white overflow-y-auto">
//           {/* ... inputs for strategy, vendor, project, poNumber ... */}
//           {/* (Note: Inputs kept same as your provided code for brevity) */}
//           <div className="space-y-5">
//             <div>
...
//                       </div>
//                       <div>
//                         {/* UPDATED: Accessing description from material relation */}
//                         <p className={`text-xs font-black ${isSelected ? 'text-indigo-900' : 'text-slate-700'}`}>
//                           {m.material?.description || 'No Description'}
//                         </p>
//                         {/* UPDATED: Accessing itemCode and unitOfMeasure from material relation */}
//                         <p className="text-[10px] font-mono text-slate-400 uppercase tracking-tighter">
//                           {m.material?.itemCode} • Qty: {m.quantityRequired} {m.material?.unitOfMeasure || 'units'}
//                         </p>
//                       </div>
...
//           {/* Footer unchanged logic, but verify variables */}
//           <div className="mt-6 pt-6 border-t flex justify-between items-center bg-slate-50/50">
//              <div className="flex items-center gap-6">
//                 <div>
//                   <p className="text-[10px] font-black text-slate-400 uppercase mb-0.5">Total Commitment</p>
...
//                 </div>
//              </div>
//              <button type="submit" disabled={loading || selectedReqIds.length === 0} className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest">
//                 {loading ? <Loader2 className="animate-spin" /> : <><ShieldCheck size={18}/> {initialData ? 'Update Ledger' : 'Authorize & Commit'}</>}
//              </button>
//           </div>
//         </div>
