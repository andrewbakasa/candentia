...
export const PROCUREMENT_SCOPES: SearchScope[] = [
    { key: 'vendorname', label: 'Vendor/Partner' },
    { key: 'itemCode', label: 'Item/Asset Code' },
    { key: 'description', label: 'Description' },
    { key: 'projectName', label: 'Project/Model Name' },
    { key: 'category', label: 'Strategic Category' },
    { key: 'status', label: 'Execution Status' },
];

const TAB_DEFAULTS: Record<string, string[]> = {
    mastermaterials: ['itemCode', 'description', 'category'],
    purchaseorders: ['vendorname', 'itemCode', 'status'],
    boq: ['projectName', 'description', 'category'],
    default: ['projectName', 'description', 'vendorname']
};

const ProcurementListView = ({ items = [], activeTab, onEdit ,permissions}: { 
    items: any[], 
    activeTab: string, 
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 space-y-4">
                <div className="flex flex-col lg:flex-row items-stretch gap-4">
                    ...
                    <div className="flex gap-3 h-16">
                        <div className="flex-1 lg:w-64 flex items-center gap-4 px-6 bg-slate-900 border border-slate-900 rounded-2xl shadow-lg">
                ...
                {filteredItems.length > 0 ? (
                    <div className="space-y-4">
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredItems.map((item) => (
                                        <DesktopRow key={item.id} item={item} onEdit={onEdit} permissions={permissions} />
                                    ))}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div className="md:hidden space-y-4">
                            {filteredItems.map((item) => (
                                <MobileCard key={item.id} item={item} onEdit={onEdit}  permissions={permissions}/>
                            ))}
                        </div>
                    </div>
                ) : (
...
const DesktopRow = ({ item, onEdit , permissions}: { item: any, onEdit: (item: any) => void , permissions:any}) => (
...
const MobileCard = ({ item, onEdit , permissions}: { item: any, onEdit: (item: any) => void,permissions:any }) => (
    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
        <div className="flex justify-between items-start">
           ...
           { permissions.canEdit && <button onClick={() => onEdit(item)} className="p-3 bg-slate-900 text-white rounded-xl shadow-md">
...
const exportToExcel = async (filteredData: any[], tabName: string) => {
    ...
    if (isMasterMaterials) {
      ...
      item.lineItems.forEach((li: any) => {
        ...
        worksheet.addRow({
          ...
          li_qty: qtyOrdered,
          li_unit: unitPrice,
          value: Number(li.totalPrice) || (qtyOrdered * unitPrice),
  ...
  saveAs(new Blob([buffer]), `NRZ_Procurement_${tabName}_${new Date().getTime()}.xlsx`);
