        ...
        <>
            <tr className="hover:bg-slate-50/50 transition-colors group">
                <td className="px-8 py-4">
                    <button onClick={onToggle} className="p-1.5 hover:bg-white rounded-lg border border-transparent hover:border-slate-200 transition-all">
                        {isExpanded ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
                    </button>
                </td>
                <td className="px-4 py-4">
                    <p className="text-sm font-black text-slate-900">{act.description}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{act.tasks?.length || 0} Tasks</p>
                </td>
                <td className="px-8 py-4">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-500">
                        <Calendar size={12}/> {new Date(act.startDate).toLocaleDateString()}
                    </div>
                </td>
                <td className="px-8 py-4"><span className="text-sm font-black text-slate-900">${(act.actualCost || 0).toLocaleString()}</span></td>
                <td className="px-8 py-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={onAddTask} className="p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-indigo-600 hover:text-white"><Plus size={14}/></button>
                        <button onClick={onEdit} className="p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-900 hover:text-white"><Edit3 size={14}/></button>
                    </div>
                </td>
            </tr>
            {isExpanded && act.tasks?.map((task: any) => (
                <tr key={task.id} className="bg-slate-50/30">
                    <td className="px-8 py-3"></td>
                    <td colSpan={3} className="px-4 py-3">
                        <div className="flex items-center gap-3">
                            {task.status === 'COMPLETED' ? <CheckCircle2 size={14} className="text-emerald-500" /> : <div className="w-3.5 h-3.5 rounded-full border-2 border-slate-300" />}
                            <span className={`text-xs font-bold ${task.status === 'COMPLETED' ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{task.title}</span>
                        </div>
                    </td>
                    <td className="px-8 py-3 text-right">
                        <button onClick={() => onEditTask(task)} className="text-[10px] font-black text-indigo-600 uppercase hover:underline">Edit</button>
                    </td>
                </tr>
            ))}
        </>
