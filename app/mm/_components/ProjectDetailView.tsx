    ...
    return (
        <div className="flex flex-col gap-2 md:gap-3 bg-slate-50/50 p-3 md:p-2 min-h-screen pb-24 lg:pb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                <div className="lg:col-span-2 bg-white rounded-[1.5rem] md:rounded-[2rem] p-4 border border-slate-200 shadow-sm">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => { setEditingRecord({ projectId: project.id }); setActiveModal('po'); }} className="p-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-lg">
                                ...
                                return (
                                    <div key={po.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                                        <div className="p-4">
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex items-start gap-2">
                                                    <button onClick={() => togglePO(po.id)} className="mt-1 p-1 bg-indigo-50 text-indigo-600 rounded-md">
                                        ...
                                        {isExpanded && itemCount > 0 && (
                                            <div className="bg-slate-50 border-t border-slate-100 p-4 space-y-2 animate-in slide-in-from-top-2">
                                                {po.lineItems.map((item: any) => (
                                                    <div key={item.id} className="flex justify-between text-[11px] bg-white p-2 rounded-lg border border-slate-100">
                                                        <div className="flex flex-col">
                                                            <span className="font-bold text-slate-700">{item.description}</span>
                                                            <span className="text-[9px] font-mono text-slate-400">{item.itemCode}</span>
                                                        </div>
                                            ...
                                            <React.Fragment key={po.id}>
                                                <tr className={`hover:bg-slate-50 transition-colors ${isExpanded ? 'bg-indigo-50/20' : ''}`}>
                                                    <td className="px-8 py-4 font-black text-slate-900">
                                                        <div className="flex items-center gap-3">
                                                            <button onClick={() => togglePO(po.id)} className="p-1 hover:bg-slate-200 rounded-md text-indigo-600">
                                                                    ...
                                                                    <tbody>
                                                                        {po.lineItems?.map((item: any) => (
                                                                            <tr key={item.id} className="border-b border-slate-50">
                                                                                ...
                                                                                <td className="px-4 py-2 text-right font-bold">${item.totalPrice?.toLocaleString()}</td>
                                                                            </tr>
                                                                        ))}
                                                                    </tbody>
                                                                </table>
                ...
                <div className="fixed inset-0 z-[150] flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="w-full max-w-2xl bg-white rounded-t-[2rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[92vh] flex flex-col animate-in slide-in-from-bottom duration-300">
                        <div className="p-2 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            ...
                            <button onClick={closeModal} className="p-2 hover:bg-slate-200 rounded-full">
