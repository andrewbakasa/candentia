...
// export default function ProjectDetailView1({ project, onRefresh, MM_ActivityForm, allStrategies = [] }: ProjectDetailViewProps) {
...
// Assuming these are your sub-components/UI kits
// Import { Badge, StatCard, MobileActivityCard, ActivityRow, MaterialCard, POCard, EmptyState } from './your-components';
...
export default function ProjectDetailView({ 
    ...
    const togglePO = (id: string) => {
        setExpandedPOs(prev => prev.includes(id) ? prev.filter(a => a !== id) : [...prev, id]);
    };
    ...
    return (
        <div className="flex flex-col gap-2 md:gap-3 bg-slate-50/50 p-3 md:p-2 min-h-screen pb-24 lg:pb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                <div className="bg-slate-900 rounded-[1.5rem] md:rounded-[2rem] p-3 text-white shadow-xl relative overflow-hidden">
                    <Target className="absolute -right-4 -bottom-4 text-white/5 w-24 h-24 md:w-32 md:h-32" />
                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-4 block">Strategic Alignment {project.plan?.year}</span>
                    <div className="space-y-4 relative z-10">
                        <div>
            ...
            <section className="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden mb-10">
                <div className="p-5 md:p-8 border-b border-slate-100">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => { setEditingRecord({ projectId: project.id }); setActiveModal('boq'); }} className="p-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2 text-[10px] font-black uppercase">
                                    <Plus size={14} className="text-indigo-600" /> Add BoQ
                                </button>
                                <button onClick={() => { setEditingRecord({ projectId: project.id }); setActiveModal('po'); }} className="p-2.5 bg-indigo-600 text-white rounded-xl shadow-lg hover:bg-indigo-700 transition-all flex items-center gap-2 text-[10px] font-black uppercase">
                                    <Package size={14} /> Issue PO
                                </button>
                            </div>
                        </div>
                ...
                <div className="bg-slate-50/30">
                    ...
                    <div className="md:hidden p-5 space-y-4">
                        {procurementTab === 'materials' ? (
                            project.materialRequirements?.length > 0 ? project.materialRequirements.map((mat: any) => (
                                <MaterialCard key={mat.id} mat={mat} onEdit={() => { setEditingRecord(mat); setActiveModal('boq'); }} onDelete={() => handleDelete(mat.id, 'materials')} />
                            )) : <EmptyState icon={<FileText size={40} />} message="No BoQ entries registered" />
                        ) : (
                            project.purchaseOrders?.length > 0 ? project.purchaseOrders.map((po: any) => (
                                <POCard key={po.id} po={po} onEdit={() => { setEditingRecord(po); setActiveModal('po'); }} onDelete={() => handleDelete(po.id, 'pos')} />
                            )) : <EmptyState icon={<Package size={40} />} message="No active purchase orders" />
                        )}
                    </div>
                        ...
                        {procurementTab === 'materials' ? (
                            <>
                                    ...
                                    {project.materialRequirements?.map((mat: any) => (
                                        <tr key={mat.id} className="hover:bg-slate-50 transition-colors text-sm">
                                            ...
                                            <td className="px-4 py-4 font-black text-indigo-600">
                                                ${(mat.quantityRequired * mat.estimatedUnitCost).toLocaleString()}
                                            </td>
                                            <td className="px-8 py-4 text-right space-x-2">
                                                <button onClick={() => { setEditingRecord(mat); setActiveModal('boq'); }} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-indigo-600">
                                                    <Edit2 size={14}/>
                                                </button>
                                                <button onClick={() => handleDelete(mat.id, 'materials')} className="p-2 hover:bg-red-50 rounded-lg text-slate-400 hover:text-red-600">
                                                    <Trash2 size={14}/>
                                                </button>
                                            </td>
                                        </tr>
                                ...
                                <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                                    <p className="text-[10px] font-bold text-slate-500 uppercase">Registry Summary</p>
                            ...
                            <table className="w-full text-left">
                                ...
                                {project.purchaseOrders?.map((po: any) => {
                                    ...
                                    const hasItems = po.lineItems && po.lineItems.length > 0;
                                    ...
                                    return (
                                        <React.Fragment key={po.id}>
                                            <tr className={`hover:bg-slate-50 transition-colors ${isExpanded ? 'bg-slate-50/50' : ''}`}>
                                                ...
                                                <td className="px-4 py-4 text-slate-600">{new Date(po.createdAt).toLocaleDateString()}</td>
                                                                ...
                                                                <tbody className="divide-y divide-slate-50">
                                                                    {po.lineItems.map((item: any) => (
                                                                        <tr key={item.id} className="text-slate-600">
                                                                            <td className="px-4 py-2 font-mono text-indigo-600">{item.itemCode}</td>
                                                                            <td className="px-4 py-2 italic">{item.description}</td>
                                                                            <td className="px-4 py-2 text-center font-bold">{item.quantityOrdered}</td>
                                                                            <td className="px-4 py-2 text-right">${item.unitPrice?.toLocaleString()}</td>
                                                                            <td className="px-4 py-2 text-right font-bold text-slate-900">${item.totalPrice?.toLocaleString()}</td>
                                                                        </tr>
                                                                    ))}
                                                                </tbody>
                                                            </table>
