 ...
 */
export const ActivityTableView2 = ({ activities, onEdit, onDelete, onAddTask, onEditTask }: any) => {
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState('');

  // 🛠️ Updated Deep Filter Logic
  const filteredActivities = activities?.filter((act: any) => {
    ...
    // 2. Check Task Level (Deep Search)
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredActivities?.map((act: any) => {
              ...
              // Optional: Highlight matching tasks specifically when searching
              const tasksToDisplay = act.tasks || [];

              return (
                <React.Fragment key={act.id}>
                  <tr 
                    className={`flex flex-col md:table-row p-5 md:p-0 bg-white md:bg-transparent rounded-2xl md:rounded-none border md:border-0 mb-3 md:mb-0 hover:bg-slate-50/50 transition-all cursor-pointer ${isExpanded ? 'ring-2 ring-indigo-500 md:ring-0 md:bg-indigo-50/20' : ''}`}
                  ...
                  >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                             <span className={`text-[8px] px-1.5 py-0.5 rounded font-black border uppercase ${
                                act.stage === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-blue-50 text-blue-700 border-blue-100'
                             }`}>
                              {act.stage}
                            </span>
                         ...
                         <div>
                            ...
                            {isOverdue ? (
                              ...
                              <span className="flex items-center gap-1 text-emerald-600 font-black text-[10px] uppercase">
                                <CheckCircle2 size={12} /> {act.actualEnd ? 'Completed' : 'On Track'}
                              </span>
                            )}
                         </div>
                         <div className="text-right md:text-left">
                            <p className="text-[8px] font-black text-slate-400 uppercase mb-1 md:hidden">Target</p>
                            <span className="text-[11px] text-slate-500 font-mono font-bold flex items-center justify-end md:justify-start gap-1">
                    ...
                    <td className="md:px-6 md:py-5 mt-4 md:mt-0">
                      <div className="flex justify-between md:flex-col md:items-start bg-slate-50 md:bg-transparent p-3 md:p-0 rounded-xl">
                        <div className="flex flex-col">
                          <span className="text-[8px] font-black text-slate-400 uppercase">Utilization</span>
                          <span className={`text-sm font-black ${totalCost > act.allocatedBudget ? 'text-red-600' : 'text-slate-900'}`}>
                            ${totalCost.toLocaleString()}
                          </span>
                        </div>
                        <span className="text-[10px] font-bold text-slate-400 flex items-center md:mt-1">
                          OF ${act.allocatedBudget?.toLocaleString()}
                        </span>
                      </div>
                    </td>
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 bg-white hover:bg-indigo-50 text-slate-500 hover:text-indigo-600 border border-slate-100 hover:border-indigo-100 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                  >
                    Full Activity Report <ExternalLink size={12} />
                  </Link>
                  {isExpanded && (
                    <tr className="bg-slate-50/50">
                      <td colSpan={5} className="p-0">
                        <div className="p-5 md:px-20 md:py-8 border-l-4 border-indigo-500 bg-white md:bg-transparent">
                          <div className="flex items-center justify-between mb-6">
                             <h4 className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">
                               <ListChecks size={16} className="text-indigo-600" /> Operational Tasks ({act.tasks?.length || 0})
                             </h4>
                             <button 
                              onClick={(e) => { e.stopPropagation(); onAddTask(act); }}
                              className="bg-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black text-white shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all uppercase flex items-center gap-2"
                             >
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {tasksToDisplay.length > 0 ? tasksToDisplay.map((task: any) => (
                              <div key={task.id} className={`bg-white p-4 rounded-2xl border flex justify-between items-start group/task hover:border-indigo-200 transition-all shadow-sm ${
                                searchTerm && (task.title?.toLowerCase().includes(searchTerm.toLowerCase()) || task.assignedTo?.toLowerCase().includes(searchTerm.toLowerCase()))
                                ? 'border-indigo-400 ring-1 ring-indigo-100' : 'border-slate-100'
                              }`}>
                                <div className="space-y-2">
                                  <p className="text-xs font-black text-slate-800 leading-tight">{task.title}</p>
                                ...
                                <button 
                                  ...
                                  className="p-2 text-slate-300 hover:text-indigo-600 opacity-0 group-hover/task:opacity-100 transition-all"
                                >
                                  <Edit3 size={14} />
                                </button>
                              </div>
                              ...
                              <div className="col-span-full py-10 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
                                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No active tasks logged</p>
                              </div>
                            )}
                          </div>
                        </div>
            ...
            {filteredActivities?.length === 0 && (
              <tr>
                <td colSpan={5} className="py-20 text-center">
                  <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No matching activities or tasks found</p>
                </td>
              </tr>
            )}
          </tbody>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredActivities?.map((act: any) => {
              ...
              return (
                <React.Fragment key={act.id}>
                  <tr 
                    className={`flex flex-col md:table-row p-5 md:p-0 bg-white md:bg-transparent rounded-2xl md:rounded-none border md:border-0 mb-3 md:mb-0 hover:bg-slate-50/50 transition-all cursor-pointer ${isExpanded ? 'md:bg-indigo-50/20' : ''}`}
                    onClick={() => setExpandedId(isExpanded ? null : act.id)}
                  >
                    <td className="hidden md:table-cell px-6 text-center">
                      {isExpanded ? <ChevronUp size={16} className="text-indigo-600" /> : <ChevronDown size={16} className="text-slate-400" />}
                        ...
                        <div className="p-5 md:px-20 md:py-8 border-l-4 border-indigo-500 bg-white">
                          <div className="flex items-center justify-between mb-6">
                             <h4 className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                               <ListChecks size={16} className="text-indigo-600" /> Tasks ({act.tasks?.length || 0})
                             </h4>
                             <button 
                              onClick={(e) => { e.stopPropagation(); onAddTask(act); }}
                              className="bg-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black text-white hover:bg-indigo-700 transition-all uppercase flex items-center gap-2"
                             >
                               <Plus size={14} /> New Task
                             </button>
                          </div>
