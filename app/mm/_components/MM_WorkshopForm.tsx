...
// Define the Enum locally or import it from your @prisma/client
// enum MM_WorkshopType {
...
export default function MM_WorkshopForm({ initialData, onClose, onSuccess }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    specialization: initialData?.specialization || MM_WorkshopType.MECHANICAL,
    ...
    <div className="w-full bg-white">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
          ...
          >
            {formData.isActive ? 'Active' : 'Under Maintenance'}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select 
              required 
              className="w-full border-2 border-slate-100 rounded-xl p-3 outline-none focus:border-indigo-500 bg-slate-50 font-bold text-slate-700 appearance-none"
              value={formData.specialization}
              onChange={(e) => setFormData({...formData, specialization: e.target.value as MM_WorkshopType})}
            >
              {Object.values(MM_WorkshopType).map((type) => (
