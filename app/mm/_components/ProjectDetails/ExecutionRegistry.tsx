                        ...
                        <div key={act.id} className="p-4 space-y-4">
                            <div className={`p-4 rounded-2xl border transition-all ${isExpanded ? 'bg-slate-50 border-indigo-100 shadow-sm' : 'bg-white border-slate-100'}`}>
                                <div className="flex justify-between items-start mb-3">
                                    <div onClick={() => toggleExpand(act.id)} className="flex-1 cursor-pointer">
                                        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1 flex items-center gap-1">
                                            Phase {phaseNumber}
                                        </p>
                                        <h3 className="text-sm font-black text-slate-900 leading-tight">{act.description}</h3>
                                    </div>
...
//                 {/* SEARCH SCOPE TOGGLES */}
//                 <div className="flex items-center gap-3">
//                     <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Search Scope:</span>
//                     {(['phase', 'task'] as SearchScope[]).map(scope => (
//                         <button
//                             key={scope}
//                             onClick={() => toggleSearchField(scope)}
...
//                                     <tr className={`hover:bg-slate-50/30 transition-all group ${isExpanded ? 'bg-slate-50/50' : ''}`}>
//                                         <td className="px-8 py-5">
//                                             <button onClick={() => toggleExpand(act.id)} className="p-2 hover:bg-white rounded-xl border border-transparent hover:border-slate-200 transition-all">
...
//                                                         <div key={task.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between group/task hover:border-indigo-300 transition-all">
//                                                             <div className="flex items-center gap-4">
//                                                                 <div className={`w-2.5 h-2.5 rounded-full ${task.status === 'COMPLETED' ? 'bg-emerald-500' : 'bg-slate-200'}`} />
//                                                                 <div>
//                                                                     <p className="text-xs font-black uppercase text-slate-800">{task.title}</p>
//                                                                     <p className="text-[9px] text-slate-400 font-bold uppercase">{task.status} | Resp: {task.assignedTo}</p>
//                                                                 </div>
//                                                             </div>
//                                                             <div className="flex gap-2">
//                                                                 <button onClick={() => { setEditingRecord({ activityId: act.id, description: `Latency: ${task.title}` }); setActiveModal('delay'); }} className="p-2 bg-rose-50 text-rose-600 rounded-xl opacity-0 group-hover/task:opacity-100 transition-all"><Clock size={14}/></button>
//                                                                 {permissions.canEdit && (
//                                                                     <button onClick={() => { setEditingRecord(task); setSelectedActivity(act); setActiveModal('task'); }} className="p-2 text-slate-300 hover:text-indigo-600"><Edit3 size={14}/></button>
//                                                                 )}
//                                                                 {permissions.canDelete && onDeleteTask && (
//                                                                     <ConfirmAction 
//                                                                         onConfirm={() => onDeleteTask(act.id, task.id)} 
...
//                     return (
//                         <div key={act.id} className="p-4 space-y-4">
//                             <div className={`p-4 rounded-2xl border transition-all ${isExpanded ? 'bg-slate-50 border-indigo-100' : 'bg-white border-slate-100'}`}>
//                                 <div className="flex justify-between items-start mb-3">
//                                     <div onClick={() => toggleExpand(act.id)} className="flex-1 cursor-pointer">
//                                         <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1">Execution Phase</p>
//                                         <h3 className="text-sm font-black text-slate-900 leading-tight">{act.description}</h3>
//                                     </div>
//                                     <div className="flex gap-1">
//                                          <button 
...
//                                 <div className="grid grid-cols-2 gap-4 mb-4">
//                                     <div>
//                                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Budget Utilization</p>
//                                         <p className="text-xs font-black text-slate-900">${(act.actualCost || 0).toLocaleString()}</p>
//                                     </div>
//                                     <div>
//                                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Timeline</p>
...
//                                 <button 
//                                     onClick={() => toggleExpand(act.id)}
//                                     className="w-full py-2 flex items-center justify-center gap-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500"
//                                 >
//                                     {isExpanded ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}
//                                     {tasksToDisplay.length} Work Packages
//                                 </button>
...
//                                             <div className="flex items-center justify-between">
//                                                 <div className="flex items-center gap-4">
//                                                     <p className="text-[9px] text-slate-400 font-bold uppercase">Resp: {task.assignedTo}</p>
//                                                     <span className={`text-[8px] font-black px-2 py-0.5 rounded-md ${task.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
//                                                         {task.status}
//                                                     </span>
//                                                 </div>
