...
function DashboardContent({ currentUser }: { currentUser: any }) {
  ...
  const fetchData0 = useCallback(async () => {
    setLoading(true);
    try {
      ...
      const results = await Promise.all(
        endpoints.map(slug => 
          fetch(`/mm/api/${slug}`).then(async res => {
            const json = await res.json();
            return Array.isArray(json) ? json : []; // Force array type
          })
        )
      );

      setData((prev: any) => {
        const batch: any = { ...prev };
        endpoints.forEach((slug, i) => { batch[slug] = results[i]; });
        return batch;
      });
    } catch (err: any) {
