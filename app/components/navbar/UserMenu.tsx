...
interface UserMenuProps {
  ...
  // handleNavigate: (href: string) => void; // Re-add if navigation is handled by parent
}
...
const UserMenu: React.FC<UserMenuProps> = ({ currentUser }) => {
  ...
  const onLogin = useCallback(() => {
    if (!currentUser) {
      loginModal.onOpen();
      setIsOpen(false); // Close the menu after opening login modal
      return;
    }
    if (currentUser && currentUser?.isAdmin) {
      rentModal.onOpen();
      setIsOpen(false); // Close the menu after opening rent modal
    }
  ...
  return (
    <div className="relative" ref={menuRef}>
      <div className="flex flex-row items-center gap-3">
        ...
        <div
          onClick={toggleOpen} // Use internal toggleOpen
          className="
            p-4
            md:py-1
            md:px-2
