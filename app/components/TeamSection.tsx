...
// --- Team Data (Kept as is for context) ---
const teamMembers: TeamMember[] = [
  {
    id: 1,
    name: "Rodreck Tavaziva",
    role: "Chairman",
    description: `B.Eng | MSOM | MBA | Experience in Distribution, Logistics, Manufacturing (Agrochemicals, Tobacco, Packaging), ICT & Finance Audit | Data Analytics (BI, Dashboards) | Seasoned solutions Architect (ERP Specialist) with over 20yrs+. Lead by bridging high-level strategy and technical execution.`,
...
`,
  ...
  {
    id: 9, // Changed from 8 to 9 to fix duplicate ID
    name: "Happson Tshuma",
...
/**
 * Helper function to determine if a member is considered 'Executive' for styling purposes.
 * This checks for 'Executive Director' or 'Chairman'.
 */
const isExecutive = (role: string): boolean => {
  const normalizedRole = role.toLowerCase();
  return !normalizedRole.includes('non-executive director') || normalizedRole.includes('chairman');
};
    ...
    <div className="border-t border-gray-200 pt-20 -mt-20" id="our-team">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {teamMembers.map((member) => {
          const isExec = isExecutive(member.role);
          
          // ** Executive Card Classes (e.g., more prominent, different colors) **
          const executiveClasses = "bg-blue-100 shadow-2xl hover:shadow-blue-300 transition-shadow duration-500 border-b-8 border-yellow-600";
          
          // ** Non-Executive Card Classes (your original, or slightly toned down) **
          const nonExecutiveClasses = "bg-gray-50 shadow-lg hover:shadow-xl border-b-4 border-yellow-500";

          // ** Image Border Classes **
          const execImageBorder = "border-blue-500 group-hover:border-yellow-500";
          const nonExecImageBorder = "border-yellow-600 group-hover:border-yellow-500";
          
          return (
            <div 
              key={member.id} 
              className={`flex flex-col items-center text-center p-6 rounded-xl transition duration-500 group ${isExec ? executiveClasses : nonExecutiveClasses}`}
            >
              {/* Image Container */}
              <div className={`relative w-32 h-32 mb-4 rounded-full overflow-hidden border-4 ${isExec ? execImageBorder : nonExecImageBorder} transition-colors duration-300`}>
                <img
                  src={member.image} 
                  alt={member.name} 
              ...
              {/* Name (Executive members get a darker text color for emphasis) */}
              <h4 className={`text-xl font-bold mb-1 ${isExec ? 'text-blue-900' : 'text-gray-900'}`}>{member.name}</h4>
              
              {/* Role (Executive members get a primary/accent color) */}
              <p className={`font-semibold mb-3 text-sm uppercase tracking-wide ${isExec ? 'text-yellow-700' : 'text-blue-700'}`}>{member.role}</p>
