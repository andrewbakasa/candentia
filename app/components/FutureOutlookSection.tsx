...
const FutureOutlookSection: React.FC = () => {
  return (
    // Corrected the placement of the JSX comment that caused the error.
    // 1. Changed ID from 'future' to 'future-outlook' to match NavLinks.jsx
    // 2. Added pt-20 and -mt-20 to ensure correct scroll positioning below a sticky header
    <div className="pt-20 -mt-20 border-t border-gray-200 bg-gray-50 p-8 rounded-xl" id="future-outlook">
          ...
          >
            ...
            <div className="relative h-64 w-full bg-yellow-600/10 flex items-center justify-center p-6 border-b-4 border-yellow-600">
              ...
              <img
                src={focus.image}
                alt={focus.title}
                className="w-full h-full object-cover opacity-70 rounded-lg shadow-inner absolute"
                style={{ top: 0, left: 0 }} // Ensure image fills container
                loading="lazy"
              />
              <focus.icon className="relative w-12 h-12 text-blue-800 opacity-90 z-10" />
            </div>
