...
import Slider from './slider'; // Assuming this is your custom Slider component
...
export const MediaModal = () => {
    ...
    useEffect(() => {
        if (status === 'success') {
            if (!cardImages || cardImages.length === 0) {
                ...
                // setShowEditCardMedia(false); // Commenting out to preserve user's last toggle state
            }
        }
    }, [status, cardImages]);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent
                // hideDefaultClose={true} // Re-enable if you still want to hide the default close button
                className="
                    w-[95%]                 /* Occupy 95% of viewport width */
                    max-w-screen-lg        /* Max width for larger screens, adjust as needed (e.g., 'md' or 'xl') */
                    ...
                    h-auto max-h-[95vh]    /* Auto height, but never exceed 95% of viewport height */
                    flex flex-col          /* Use flexbox for direct children to manage layout */
                    [&>button:last-child]:hidden /* Hides the default close button if hideDefaultClose is not used */
                "
            >
                {/* Header section */}
                ...
                <div className="mt-6 px-2 sm:px-6 flex-grow overflow-y-auto"> {/* Added flex-grow and overflow-y-auto */}
